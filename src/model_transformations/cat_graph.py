...
class CatGraph():
    ...
    def __init__(self, name, database, rdf_database_uri, persistent = True):
        ...
        if persistent:
            store = plugin.get("SQLAlchemy", Store)(identifier="cat_graph_test")
        ...
        for table_name in edge_schema:
            ...
            if len(edge_schema[table_name]) != 0:
                ...
                for key_info in list_of_keys:
                    ...
                    result_list_of_lists = self.database.query("SELECT a." + primary_key + ", b." + foreign_key + " from " + key_info[2] + " AS a JOIN " + key_info[0] + " AS b ON a." + primary_key + "= b." + foreign_key +";")
                    for result in result_list_of_lists:
                        subject = URIRef(database_name + "/" + table_name + "/" + foreign_key + "/" + str(result[0]))
                        predicate = URIRef(database_name + "/" + table_name + "/" + foreign_key + "/" + main_table + "/" + primary_key)
                        object_elem = URIRef(database_name + "/" + table_name + "/" + primary_key + "/" + str(result[1]))
    ...
    def transform_node(self, node):
        ...
        id_value = node.id
        central_subject = URIRef(database_name + label_path + "/" + str(id_value))
    ...
    def transform_from_property_graph(self):
        result = self.database.execute_read("MATCH (n) -[r]-> (m) RETURN n, r, m")
        for elem in result:
