...
class CommonTableExpr:
    ...
    def __init__(self, raw_cte):
        ...
        self.aliascolnames = [name["String"]["str"] for name in raw_cte["aliascolnames"]]
        ...
        self.ctequery = SelectStmt(raw_cte["ctequery"]["SelectStmt"])
        self.location = raw_cte["location"]

    def transform_into_cypher(self):
        res = ""
        res += self.ctequery.transform_into_cypher()
        res += "collect({ " + ", ".join(self.aliascolnames) + "}) AS " + self.ctename + "\n"
        return res
