...
strangers AS (
   SELECT
      DISTINCT fp1.fp_personid AS personid
   FROM
      tagclass tc1,
      tagclass tc2,
      tag t1,
      ...
      forum_person fp2
   WHERE
      tc1.tc_tagclassid = t1.t_tagclassid
      ...
      AND fp1.fp_personid = fp2.fp_personid -- filter
      AND tc1.tc_name = 'MusicalArtist'
      AND tc2.tc_name = 'OfficeHolder'
),
...
interactions_longest AS (
   ...
   WHERE
      i1.strangerid IS NULL
),
interactions_between_non_friends AS (
   SELECT
      i.*
   FROM
      interactions_longest i
      LEFT JOIN knows k ON (
         i.strangerid = k.k_person1id
         AND i.replyAuthorId = k.k_person2id
      )
   WHERE
      k.k_person1id IS NULL
)
SELECT
...
FROM
   person p
   LEFT JOIN interactions_between_non_friends i ON (
      p.p_personid = i.replyAuthorId
      AND p.p_personid != i.strangerid
   )
WHERE
   p.p_birthday > '1989-01-01T00:00:00.000+00:00' :: timestamp
GROUP BY
   p.p_personid
ORDER BY
