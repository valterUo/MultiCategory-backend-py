...
WITH RECURSIVE reply_scores(
   ...
   WHERE
      ...
      AND p.m_messageid = c.m_c_replyof -- filter
      AND f.f_creationdate BETWEEN '2010-11-01T00:00:00.000+00:00' :: timestamp
      AND '2010-12-01T00:00:00.000+00:00' :: timestamp
      AND p.m_c_replyof IS NULL -- post, not comment
   UNION
   ALL
   SELECT
      r.r_threadid AS r_threadid,
...
person_pair_scores_directed AS (
   SELECT
      r_orig_personid AS orig_personid,
      r_reply_personid AS reply_personid,
      sum(r_score) AS score
   FROM
      reply_scores
   WHERE
      1 = 1 -- discard self replies from the score earned
...
person_pair_scores AS (
   ...
   SELECT
      coalesce(s1.orig_personid, s2.reply_personid) AS person1id,
      coalesce(s1.reply_personid, s2.orig_personid) AS person2id,
      coalesce(s1.score, 0.0) + coalesce(s2.score, 0.0) AS score
   FROM
      person_pair_scores_directed s1 FULL
      JOIN person_pair_scores_directed s2 ON (
         s1.orig_personid = s2.reply_personid
...
wknows AS (
   ...
   FROM
      knows k
      LEFT JOIN person_pair_scores pps ON (
         k.k_person1id = pps.person1id
         AND k.k_person2id = pps.person2id
      )
),
paths(
   startPerson,
   ...
   WHERE
      1 = 1
      AND k_person1id = 19791209303405
   UNION
   ALL
   SELECT
      p.startPerson AS startPerson,
      k_person2id AS endPerson,
