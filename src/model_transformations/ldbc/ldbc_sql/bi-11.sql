/* Q11. Unrelated replies
 \set country  '\'Germany\''
 \set blacklist '\'{"also","Pope","that","James","Henry","one","Green"}\''::text[]
 */
WITH replies_w_tags AS (
  SELECT
    p.p_personid AS creatorid,
    r.m_messageid AS replyid,
    r.m_c_replyof AS replyof,
    r.m_content AS content,
    CASE
      WHEN count(pt.mt_tagid) = 0 THEN ARRAY [] :: bigint [] -- no tags for the message, so we return an empty array
      ELSE array_agg(pt.mt_tagid)
    END AS replyTags
  FROM
    place co -- country
,
    place ci -- city
,
    person p,
    message r -- reply
    LEFT JOIN message_tag pt ON (r.m_messageid = pt.mt_messageid)
  WHERE
    1 = 1 -- join
    AND co.pl_placeid = ci.pl_containerplaceid
    AND ci.pl_placeid = p.p_placeid
    AND p.p_personid = r.m_creatorid -- filter
    AND co.pl_name = 'Germany'
    AND r.m_c_replyof IS NOT NULL
  GROUP BY
    p.p_personid,
    r.m_messageid,
    r.m_c_replyof,
    r.m_content
),
replies_blacklist_excluded_1 AS (
  SELECT
    *
  FROM
    replies_w_tags
  WHERE
    1 = 1
    AND NOT string_to_array(
      trim(
        regexp_replace(content, E '[[:space:],.?!()\r\n]+', ' ', 'g')
      ),
      ' '
    ) & & :blacklist
),
replies_blacklist_excluded_2 AS (
  SELECT
    *
  FROM
    replies_w_tags r
    LEFT JOIN unnest(
      '{"also","Pope","that","James","Henry","one","Green"}' :: text []
    ) AS bl(word) ON (r.content LIKE '%' || bl.word || '%')
  WHERE
    1 = 1 -- exclude messages by the blacklist.
    AND bl.word IS NULL
),
replies_not_sharing_tags_w_base_message AS (
  SELECT
    r.replyid,
    r.creatorid
  FROM
    replies_blacklist_excluded_2 r,
    message b -- base message of the reply
    LEFT JOIN message_tag pt ON (b.m_messageid = pt.mt_messageid)
  WHERE
    1 = 1 -- join
    AND r.replyof = b.m_messageid -- filter
  GROUP BY
    r.replyid,
    r.creatorid,
    r.replyTags
  HAVING
    NOT r.replyTags & & CASE
      WHEN count(pt.mt_tagid) = 0 THEN ARRAY [] :: bigint [] -- no tags for the message, so we return an empty array
      ELSE array_agg(pt.mt_tagid)
    END
)
SELECT
  r.creatorid AS "person.id",
  t.t_name AS "tag.name",
  count(l.l_messageid) AS likeCount,
  count(DISTINCT r.replyid) AS replyCount
FROM
  replies_not_sharing_tags_w_base_message r
  LEFT JOIN likes l ON (r.replyid = l.l_messageid),
  message_tag pt,
  tag t
WHERE
  1 = 1 -- join
  AND r.replyid = pt.mt_messageid
  AND pt.mt_tagid = t.t_tagid
GROUP BY
  r.creatorid,
  t.t_name
ORDER BY
  likeCount DESC,
  r.creatorid,
  t.t_name
LIMIT
  100;