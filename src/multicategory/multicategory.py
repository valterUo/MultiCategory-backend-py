...
class MultiCategory():
    ...
    def __init__(self, initial_db):
        ...
        predefined_multi_model_dbs = [
            ecommer_db,
            patent_db,
            MultiModelDB("Online market place multi-model database", [], []),
            MultiModelDB("Small Unibench multi-model database", [], []),
            MultiModelDB("University multi-model database", [], []),
            MultiModelDB("Person multi-model database", [], []),
            MultiModelDB("Film multi-model database", [], [])
        ]
    ...
    def change_to_multi_model_db(self, db):
        if db in self.multi_model_databases.keys():
            self.selected_multi_model_database = self.multi_model_databases[db]
        elif db in self.multi_model_databases.values():
            self.selected_multi_model_database = db
